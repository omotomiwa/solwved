<template>
  <q-page :class="ClassCombinations?.pageContainer">
    <div class="column w-standard q-pa-xs-md q-pa-lg-lg q-mt-xs-md q-mt-lg-xs swlvd-container-bg">
      <div class="text-h2 text-weight-bold text-center text-white">Services pending approval</div>
      ...
      <div v-if="isAuthorised">
        ...
        <div v-if="props.vendor_id" class="row q-col-gutter-lg q-mt-lg show-empty-message">
          <organisation-services-card
            v-for="customer in customers"
            :key="customer.id"
            :organisation="customer"
            viewing-as-type="vendor"
            :valid-delivery-model-ids="[3, 4]"
            :valid-approval-status="0"
            class="col-xs-12 col-sm-6 col-md-4 col-lg-4"
          />
        </div>
        <div v-if="props.customer_id" class="row q-col-gutter-lg q-mt-lg show-empty-message">
          <organisation-services-card
      ...
      <div v-else class="text-white q-my-md">You are not authorised to view this page</div>
    </div>
  </q-page>
</template>
...
const pageMessage = computed(() => {
  let m = ""
  if (props.customer_id) {
    m = "This page lists pending SOW and WPO services where your organisation is acting as the buyer. Once a service has been approved it will become an Active Service."
  } else if (props.vendor_id) {
    m = "This page lists pending SOW and WPO services where your organisation is acting as the vendor. Once a service has been approved it will become an Active Service."
  }
  return m
})
const contracts = computed(() => store.getters["contract/getOrganisationContracts"](organisation.value.id))
...
.show-empty-message:empty:after {
  content: "There are no pending services requiring your approval";
  display: block;
  color: white;
  padding-left: 24px;
}
