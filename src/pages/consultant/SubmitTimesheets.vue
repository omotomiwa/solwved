<template>
  <q-page >
    <div class="column w-standard q-pa-xs-md q-pa-lg-lg q-mt-xs-md q-mt-lg-xl swlvd-container-bg">
        ...
        <div class="text-h2 text-weight-bold text-center text-white q-mt-md">Submit Timesheet</div>
      ...
      <div class="row justify-between">
        ...
        <router-link :to="timesheetLink(engagement_id)" class="text-primary "  style="text-decoration: none">
...
export default defineComponent({
  ...
  props: ["engagement_id","engagements"],
  setup() {
    ...
    const invoiceColumns = ref([
      ...
      { name: "Vendor", required: true, label: "Vendor", align: "left", field: "vendor_name", sortable: true },
  ...
  created() {
    ...
    if (engagements.length > 0) {
     let vendorId = engagements[0].vendor_id;

      // Step 3: Fetch copy week
     

      // Step 4: Now fetch organisation
      const orgData = await this.$store.dispatch("consultant/getOrganisation", vendorId);
    ...
    this.$store
  .dispatch("consultant/getInvoices", this.engagement_id)
  .then((invoices) => {
  ...
  return "/consultant"
     ...
     timesheetLink(engagement_id) {
  return "/timesheet-records/" + engagement_id
}
,
    handleGenerateInvoice() {
      ...
      this.$store
        .dispatch("consultant/generateInvoice", payload)
        .then(() => {
          this.$q.notify({
            type: "positive",
          ...
          return  this.$store.dispatch("consultant/getInvoices", this.engagement_id)
        })
        .then((invoices) => {
          this.invoices = invoices
        })
