<template>
  <q-page :class="ClassCombinations?.pageContainer">
    <div v-if="allowNewRecord || action === 'edit'"
      ...
      <div>
        <div class="text-h6 text-weight-bold q-mb-md">{{ page_title }}: {{ selectedWeekLabel }}</div>
      </div>
        ...
        <div class="column q-pa-xs-sm q-pa-md-md q-pa-lg-lg no-wrap">
          ...
          <div>
            ...
            <div class="row items-center no-wrap q-mt-lg q-mb-sm">
              <div class="col">
                <div class="text-h6">Output Rows</div>
              </div>
            </div>
            ...
            />
            <q-btn
              v-if="allowChanges && !fallowWeek"
              color="primary"
              text-color="dark"
              label="Add row"
              icon="add"
              :disable="!recordsComplete"
              class="q-mb-md q-mt-md add-row"
              @click="addOutputRow"
            />
      ...
      <div class="gt-xs row q-mt-md q-mb-xl" :class="action === 'new' ? 'justify-between' : 'justify-between'">
        <q-btn
          :label="$q.screen.gt.xs ? 'Back to list' : 'Back'"
          color="white"
          outline
          icon="reply"
          class="q-mr-md"
        ...
        <q-btn-dropdown
          ...
          :label="$q.screen.gt.xs ? 'Copy week' : 'Copy week'"
          color="primary"
          outline
          icon="content_copy"
          class="q-mr-md"
        ...
        <q-btn-dropdown
          ...
          :label="$q.screen.gt.xs ? 'Fallow week' : 'Fallow week'"
          color="accent"
          icon="flaky"
        >
          <q-list>
        ...
        <div class="row">
          <q-btn
            v-if="action === 'edit'"
            class="q-mr-md"
            :label="$q.screen.gt.xs ? 'REVERT CHANGES' : 'CANCEL'"
            icon="close"
            color="white"
            outline
            :disable="!allowRevert"
          ...
          />
          <q-btn
            :label="$q.screen.gt.xs ? 'SAVE RECORD' : 'SAVE'"
            icon="save"
            color="primary"
            text-color="dark"
            :disable="!allowSave"
    ...
    <div v-else class="w-standard q-pa-xs-xs q-pa-lg-lg q-mt-xs-md q-mt-lg-xl swlvd-container-bg">
      ...
      <div v-if="engagement" class="column">
        <div class="column q-pa-xs-sm q-pa-md-md q-pa-lg-lg no-wrap">
          You can't add a new delivery record at this point, as you have already completed all records to date. Please
...
export default defineComponent({
  name: "DeliveryRecord",
  components: { OutputRow, OutputRequestForm, NewSelectorForm },
  ...
  computed: {
    ...
    totalEngagementEffort: function() {
      let units = 0
    ...
    availableUnitsExceeded: function() {
    ...
    combinedRecord: function() {
      ...
      if (this.engagement) {
        record = {
          ...
          week: this.selectedWeekNumber ? parseInt(this.selectedWeekNumber.split("-")[1]) : null,
