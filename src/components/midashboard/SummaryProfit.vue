...
const totalPeriodProfit = computed(() =>
  CurrencyMethods.toCurrencyFormat(organisation.value?.currency?.code, periodProfit.value)
)
...
function buildData() {
  ...
  periodProfit.value = revenue - costs
}
...
<template>
  <summary-widget :value="totalPeriodProfit.toString()" label="Total gross profit" />
</template>
