...
const totalPeriodRevenue = computed(() => CurrencyMethods.toCurrencyFormat(organisation.value?.currency?.code, periodRevenue.value))
...
<template>
  <summary-widget
    :value="totalPeriodRevenue.toString()"
    :label="view === 'customer' ? 'Total spend' : 'Total charges'"
  />
</template>
