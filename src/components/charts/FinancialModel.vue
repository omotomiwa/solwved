<template>
  <v-chart v-if="!noData" class="chart" ref="chart" :manual-update="true" />
...
const totalsByFinancialModel = computed(() => {
  const dm = {}
  if (props.reportsData && props.reportsData.length) {
    for (let i = 0; i < props.reportsData.length; i++) {
      const report = props.reportsData[i]
      if (dm[report.service.financial_model_id] === undefined) {
        dm[report.service.financial_model_id] = {
          name: report.service.financial_model.name,
          total: 0
        }
      }
      dm[report.service.financial_model_id].total += getReportTotal(report)
    }
  }
  return Object.values(dm)
})
const financialModelNames = computed(() => {
  let names = []
  if (totalsByFinancialModel.value.length) {
    names = totalsByFinancialModel.value.map(elem => {
      return elem.name
    })
  }
  return names
...
function buildOption() {
  return {
    ...
    tooltip: {
      ...
      formatter: params => {
        return (
          params.data.name +
          "<br/>" +
          CurrencyMethods.toCurrencyFormat(organisation.value?.currency?.code, params.data.value) +
          " (" +
          params.percent +
          "%)"
        )
    ...
    legend: {
      orient: "vertical",
      left: "left",
      data: financialModelNames.value
    },
    series: [
      {
        ...
        center: ["50%", "60%"],
        startAngle: 315,
        data: totalsByFinancialModel.value.map(elem => {
          return {
            name: elem.name,
            value: elem.total
          }
